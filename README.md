...
## 문자(SMS) 알림 설정 (신청/승인/반려/5분전 알림)

이 앱은 알리고(Aligo) SMS API를 통해 문자를 보냅니다. 아래 절차를 순서대로 진행해주세요.

### 1. 알리고 준비물 확보
- smartsms.aligo.in 가입, 최소 5만원 충전
- 발신번호 등록 (통신서비스 이용증명원 제출)
- API Key 발급 (로그인 후 [API Key 발급 신청])
- 아이디, API Key, 발신번호(하이픈 없이) 3가지를 메모

### 2. Supabase CLI 설치 및 로그인 (최초 1회)
```bash
...
```
(`YOUR-PROJECT-REF`는 Supabase 프로젝트 URL의 `https://이부분.supabase.co`에서 확인)

### 4. 시크릿(비밀값) 등록
```bash
supabase secrets set ALIGO_API_KEY=발급받은키
supabase secrets set ALIGO_USER_ID=알리고아이디
supabase secrets set ALIGO_SENDER=발신번호숫자만
```
...
### 6. 5분 전 알림 스케줄러 등록
1. `supabase/cron-schedule.sql` 파일을 열어서 `YOUR-PROJECT-REF`와 `YOUR-ANON-KEY`를 실제 값으로 바꿔주세요 (anon key는 Settings → API에서 확인)
2. Supabase SQL Editor에 붙여넣고 실행

### 7. 테스트
- 앱에서 배차 신청 → 차량국장 번호로 문자가 오는지 확인
- 관리자 모드에서 승인/반려 → 신청자 번호로 문자가 오는지 확인
- 5분 전 알림은 스케줄러 등록 후 최대 1분 이내에 매분 자동 실행되며, 실제 예약 5분 전에 문자가 옵니다

### 문제가 생기면
```bash
supabase functions logs send-sms
supabase functions logs reminder-check
```
로 에러 로그를 확인할 수 있습니다. 로그를 캡처해서 보내주시면 원인을 봐드리겠습니다.
