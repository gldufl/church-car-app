...
이 앱은 알리고(Aligo) SMS API를 통해 문자를 보냅니다.
- **배차 신청 시** → 문자 수신을 선택한 관리자(들)에게 발송
- **배차 승인 시** → 신청한 운전자에게 발송

### 1. 알리고 준비물 확보
- smartsms.aligo.in 가입, 최소 5만원 충전
- 발신번호 등록 (통신서비스 이용증명원 제출)
- API Key 발급
- 아이디, API Key, 발신번호(하이픈 없이) 메모
...
### 6. 앱에서 문자 수신 관리자 설정
1. 관리자 모드 → **운전자 관리** 탭
2. 문자를 받을 관리자 계정의 **수정** 버튼 → "문자 수신 번호" 입력 → 저장
3. 같은 화면에서 **"🔔 새 배차 신청 문자 받기"** 체크박스 켜기 (여러 관리자를 동시에 선택할 수 있습니다)

### 7. 테스트
- 운전자 계정으로 배차 신청 → 선택한 관리자 번호로 문자가 오는지 확인
- 관리자 모드에서 승인 → 신청자 번호로 문자가 오는지 확인
